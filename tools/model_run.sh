...
if [[ "$OUTPUT_BUCKET" != "benchmark" && "$OUTPUT_BUCKET" != "ml-data" && "$OUTPUT_BUCKET" != "history" ]]; then
  echo "Invalid output bucket: $OUTPUT_BUCKET (benchmark|ml-data|history)" >&2
  exit 2
fi
